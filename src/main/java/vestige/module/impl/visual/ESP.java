...
public class ESP extends Module {
    ...
    private void renderNametag(EntityLivingBase entity, VestigeFontRenderer fontRenderer, float x, float y, float right, float bottom) {
        ...
        if (healthValue > 0.75f) {
            healthColor = new Color(66, 246, 123);
        } else if (healthValue > 0.5f) {
            healthColor = new Color(228, 255, 105);
        } else if (healthValue > 0.35f) {
            healthColor = new Color(236, 100, 64);
        } else {
            healthColor = new Color(255, 65, 68);
        }
        ...
        float middle = x + ((right - x) / 2f);
        float textWidth = fontRenderer.getStringWidth(text.toString());
        middle -= (textWidth * (float)fontScale) / 2f;
        double fontHeight = fontRenderer.getHeight() * fontScale;
        ...
        if (background.isEnabled()) {
            if (rounded.isEnabled()) {
                RenderUtil.drawRoundedRect(middle - 3, (float)(y - (fontHeight + 7)), textWidth + 6,
                        (float)((fontHeight / fontScale) + 4), 4, backgroundColor.getRGB());
            } else {
        ...
        if (mcfont.isEnabled()) {
            mc.fontRendererObj.drawString(StringUtils.stripControlCodes(text.toString()),
                    middle + 0.5f, (float)(y - (fontHeight + 4)) + 0.5f, Color.BLACK.getRGB());
            mc.fontRendererObj.drawString(text.toString(), middle, (float)(y - (fontHeight + 4)), healthColor.getRGB());
        } else {
            fontRenderer.drawStringWithShadow(text.toString(), middle, (float)(y - (fontHeight + 5)), healthColor.getRGB());
        }
    ...
    private void renderHealthBar(EntityLivingBase entity, float x, float y, float bottom) {
        ...
        if (healthBarText.isEnabled()) {
            VestigeFontRenderer fontRenderer = mcfont.isEnabled() ? mc.fontRendererObj : font;
            healthValue *= 100;
            String health = String.valueOf(Math.round(healthValue * 10) / 10.0).substring(0, healthValue == 100 ? 3 : 2);
            String text = health + "%";
            float fontScale = 0.5f;
            ...
            float textY = y + (newHeight - (newHeight * (healthValue / 100f)));
