    ...
    public void renderModule(boolean inChat) {
        ...
        for(AnimationHolder<KeyBinding> key : keys) {
            ...
            if(index == 4) {
                ...
                RenderUtil.drawRoundedRect(currentX, currentY, totalLength, length * 0.85F, 4, new Color(40, 40, 45, 220).getRGB());
                ...
                if(key.isRendered() || !key.isAnimDone()) {
                    key.render(() -> RenderUtil.drawRoundedRect(finalRenderX, finalRenderY, totalLength, length * 0.85F, 4, theme.getColor(100 + finalRenderX + length + spacing)), finalRenderX, finalRenderY, finalRenderX + totalLength, finalRenderY + length * 0.85F);
                }
                ...
                RenderUtil.drawRoundedRect(currentX, currentY, length, length, 4, new Color(40, 40, 45, 220).getRGB());
                ...
                if(key.isRendered() || !key.isAnimDone()) {
                    key.render(() -> RenderUtil.drawRoundedRect(finalRenderX, finalRenderY, length, length, 4, theme.getColor(100 + finalRenderX * 3)), finalRenderX, finalRenderY, finalRenderX + length, finalRenderY + length);
                }
